...
Fingerprint.prototype.writeRailsManifest = function(destDir, noFingerprint) {
    ...
    md5.update(contents);
    hash = md5.digest('hex');
    var suffix = noFingerprint ? '' : '-' + hash;
    fs.writeFileSync(destDir + '/assets/manifest' + suffix + '.json', contents);
};
...
Fingerprint.prototype.write = function(readTree, destDir) {
  ...
  return Filter.prototype.write.apply(this, arguments).then(function() {
    if (!!self.generateRailsManifest) {
      var noFingerprint = self.exclude.indexOf('manifest.json') !== -1;
      self.writeRailsManifest(destDir, noFingerprint);
    }
  });
};
